...
public class Level4ChunkGen extends AbstractNbtChunkGenerator {

    public static final Codec<Level4ChunkGen> CODEC = RecordCodecBuilder.create((level4ChunkGenInstance -> level4ChunkGenInstance.group(BiomeSource.CODEC.fieldOf("biome_source").stable().forGetter(
    ...
    public Level4ChunkGen(BiomeSource biomeSource, long worldSeed) {
        super(new SimpleRegistry<StructureSet>(Registry.STRUCTURE_SET_KEY, Lifecycle.stable(), null), Optional.empty(), biomeSource, biomeSource, worldSeed, BackroomsMod.id("level_4"), LiminalUtil.createMultiNoiseSampler());    }
    ...
    public ChunkGenerator withSeed(long seed) {
        return new Level4ChunkGen(this.biomeSource,seed);
    }
    ...
    public CompletableFuture<Chunk> populateNoise(ChunkRegion chunkRegion, ChunkStatus targetStatus, Executor executor, ServerWorld world, ChunkGenerator generator, StructureManager structureManager, ServerLightingProvider lightingProvider, Function<Chunk, CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>>> function, List<Chunk> chunks, Chunk chunk, boolean bl) {
        ...
        if (mod(x, 48) == 0 && mod(z, 48) == 0) {
            ...
            else if (random.nextInt(100) >=7 && random.nextInt(100) < 35) {
