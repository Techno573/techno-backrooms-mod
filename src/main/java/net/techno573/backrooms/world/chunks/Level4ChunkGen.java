...
public class Level4ChunkGen extends NbtChunkGenerator {

    public static final Codec<Level4ChunkGen> CODEC = RecordCodecBuilder.create((level4ChunkGenInstance -> {
        return level4ChunkGenInstance.group(BiomeSource.CODEC.fieldOf("biome_source").stable().forGetter(
                (Level4ChunkGen) -> Level4ChunkGen.biomeSource
        ),Codec.LONG.fieldOf("seed").stable().forGetter((Level4ChunkGen) -> Level4ChunkGen.worldSeed)).apply(
                level4ChunkGenInstance,level4ChunkGenInstance.stable(Level4ChunkGen::new));
    }));

    public Level4ChunkGen(BiomeSource biomeSource, long worldSeed) {
        super(biomeSource, worldSeed, BackroomsMod.id("level_4"),List.of());
    }

    public Level4ChunkGen(BiomeSource biomeSource, MultiNoiseUtil.MultiNoiseSampler multiNoiseSampler, long worldSeed, Identifier nbtId, List<String> structures) {
    ...
    public CompletableFuture<Chunk> populateNoise(Executor executor, Chunk chunk, ChunkStatus targetStatus, ServerWorld world, ChunkRegion chunkRegion, StructureManager structureManager, ServerLightingProvider lightingProvider) {
        ...
        if (mod(x, 48) == 0 && mod(z, 48) == 0) {
            ...
            else if (random.nextInt(100) >=7 && random.nextInt(100) < 45) {
    ...
    public ChunkGenerator withSeed(long seed) {
        return new Level4ChunkGen(biomeSource,seed);
    }
