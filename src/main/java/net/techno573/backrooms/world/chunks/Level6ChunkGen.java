...
public class Level6ChunkGen extends AbstractNbtChunkGenerator {

    public static final Codec<Level6ChunkGen> CODEC = RecordCodecBuilder.create((level6ChunkGenInstance -> level6ChunkGenInstance.group(BiomeSource.CODEC.fieldOf("biome_source").stable().forGetter(
            (Level6ChunkGen) -> Level6ChunkGen.biomeSource
    ),Codec.LONG.fieldOf("seed").stable().forGetter((Level6ChunkGen) -> Level6ChunkGen.worldSeed)).apply(
            level6ChunkGenInstance,level6ChunkGenInstance.stable(Level6ChunkGen::new))));

    private long worldSeed;

    public Level6ChunkGen(BiomeSource biomeSource, long worldSeed) {
        super(new SimpleRegistry<StructureSet>(Registry.STRUCTURE_SET_KEY, Lifecycle.stable(), null), Optional.empty(), biomeSource, biomeSource, worldSeed, BackroomsMod.id("level_6"), LiminalUtil.createMultiNoiseSampler());    }
    ...
    public ChunkGenerator withSeed(long seed) {
        return new Level6ChunkGen(this.biomeSource,seed);
    }
