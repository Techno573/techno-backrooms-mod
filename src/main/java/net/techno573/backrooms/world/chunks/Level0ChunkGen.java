...
public class Level0ChunkGen extends NbtChunkGenerator {
    ...
    public Level0ChunkGen(BiomeSource biomeSource, long worldSeed) {
        super(biomeSource, worldSeed, BackroomsMod.id("level_0"),List.of());
    }

    public Level0ChunkGen(BiomeSource biomeSource, MultiNoiseUtil.MultiNoiseSampler multiNoiseSampler, long worldSeed, Identifier nbtId, List<String> structures) {
    ...
    public CompletableFuture<Chunk> populateNoise(Executor executor, Chunk chunk, ChunkStatus targetStatus, ServerWorld world, ChunkRegion chunkRegion, StructureManager structureManager, ServerLightingProvider lightingProvider) {
        ...
        if (mod(x, 48) == 0 && mod(z, 48) == 0) {
            ...
            else if (random.nextInt(100) >=5 && random.nextInt(100) < 30) {
    ...
    public ChunkGenerator withSeed(long seed) {
        return new Level0ChunkGen(biomeSource,seed);
    }
