...
public class LevelFunChunkGen extends AbstractNbtChunkGenerator {

    public static final Codec<LevelFunChunkGen> CODEC = RecordCodecBuilder.create((levelFunChunkGenInstance -> levelFunChunkGenInstance.group(BiomeSource.CODEC.fieldOf("biome_source").stable().forGetter(
            (LevelFunChunkGen) -> LevelFunChunkGen.biomeSource
    ),Codec.LONG.fieldOf("seed").stable().forGetter((LevelFunChunkGen) -> LevelFunChunkGen.worldSeed)).apply(
            levelFunChunkGenInstance,levelFunChunkGenInstance.stable(LevelFunChunkGen::new))));

    private long worldSeed;

    public LevelFunChunkGen(BiomeSource biomeSource, long worldSeed) {
        super(new SimpleRegistry<StructureSet>(Registry.STRUCTURE_SET_KEY, Lifecycle.stable(), null), Optional.empty(), biomeSource, biomeSource, worldSeed, BackroomsMod.id("level_fun"), LiminalUtil.createMultiNoiseSampler());    }
    ...
    public ChunkGenerator withSeed(long seed) {
        return new LevelFunChunkGen(this.biomeSource,seed);
    }
